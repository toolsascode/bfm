...
func (s *Server) Migrate(ctx context.Context, req *MigrateRequest) (*MigrateResponse, error) {
	...
	// Resolve schema (use environment if provided for dynamic schemas)
	schema := req.Schema
	if schema == "" && req.Environment != "" {
		// For dynamic schemas, use environment to build schema name
		// e.g., "cli_{environment_id}"
		schema = fmt.Sprintf("cli_%s", req.Environment)
	}
...
func (s *Server) StreamMigrate(req *MigrateRequest, stream MigrationService_StreamMigrateServer) error {
	...
	for i, migration := range migrations {
		migrationID := fmt.Sprintf("%s_%s_%s_%s", migration.Schema, migration.Table, migration.Version, migration.Name)
